...
void solve()
{
    ...
    ll ct=0;
//    f(i,1,n)if(v[i]<v[i-1]){auto it=lower_bound(all(two),v[i-1]-v[i])-two.begin(); ct+=it+1;}
    vl suf(n);
    suf[n-1]=v[n-1];
    of(i,n-2,0)
        {
            suf[i]=min(v[i+1],suf[i+1]);
        }
    f(i,0,n)
        {
            if(v[i]>suf[i])
                {
                    ct=max(ct,(ll)ceil(log2(v[i]-suf[i]+1)));
                }
        }
    cout<<ct<<endl;
}
